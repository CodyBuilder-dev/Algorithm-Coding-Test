...
def reverse(reverse_tree,current,gift):
    if current == "-" or gift == 0:
        return

    reverse_tree[current]["revenue"] += gift - gift//10
    reverse(reverse_tree,reverse_tree[current]["parent"],gift//10)

def create_reverse_tree(enroll, referral, seller, amount):
    tree = dict.fromkeys(["-"] + enroll)
    for k in tree:
        tree[k] = {"revenue": 0, "parent": None}
...
def solution(enroll, referral, seller, amount):
    tree = create_reverse_tree(enroll, referral, seller, amount)


    for s,a in zip(seller,amount):
        reverse(tree,s,a*100)
