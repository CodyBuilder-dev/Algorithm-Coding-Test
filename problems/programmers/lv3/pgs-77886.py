...
최적화 :
- 스택을 이용하면, 특정 문자열에서 특정 문자열을 재귀적으로 뽑아낼 수 있습니다
- deque를 사용하면 왼쪽부터 pop하는 경우에 효율적입니다
"""
...
def solution(s):
    ...
    for x in s:
        temp = x
        left = []
        right = deque(x) # 0부터 pop해야 하므로 list를 쓰면 안됨
        stack = []
        ...
        while right:
            ...
            if left[-3:] == ["1","1","0"]:
                stack.append("110")
                left.pop(); left.pop(); left.pop() # left = left[:-3]따위로 쓰면 안됨

        temp = ''.join(left) + "1" * 10**6
        pos = temp.find("111")
        temp = temp[:pos] + "110"*len(stack) + temp[pos:len(x)] # 위에서 10^6으로 늘렸으므로 temp[pos:] 처럼 끝까지 탐색해선 안됨
