...
def solution(play_time, adv_time, logs):
    ...
    for log in sorted(logs):
       ...
       # 1. view cnt를 셀 때 얼마나 효율적으로 잘 세느냐가 시간의 성공 싸움
       #  for i in range(start, end):
       #      view_cnt[i] += 1
        view_cnt[start] += 1
        if end < play_time:
            view_cnt[end+1] -= 1
    # 아래와 같이 len으로 해버리면, play_time과 갑이 달라지므로 혼돈이 생긴다
    # for i in range(1,len(view_cnt)):
    # 아래와 같이 play_time을 반복문의 기준으로 해주는 것이 좋다
    for i in range(1, play_time+1):
        view_cnt[i] += view_cnt[i-1]

    max_cnt = sum(view_cnt[:adv_time+1])
    max_idx = 0
    temp_cnt = max_cnt

    # 2. max_cnt를 셀 때 얼마나 효율적으로 잘 세느냐가 시간의 성공 싸움
    for j in range(1, play_time+1-adv_time):
        temp_cnt = temp_cnt - view_cnt[j-1] + view_cnt[j+adv_time]
        if max_cnt < temp_cnt:
            max_idx = j
            max_cnt = temp_cnt

    t,seconds  = max_idx // 60, max_idx % 60
...
print(solution("00:00:02", "00:00:01", ["00:00:01-00:00:02","00:00:00-00:00:02","00:00:00-00:00:01","00:00:00-00:00:01"]))